...
export default async function apiGetStandardLabelUrlsSwift(context, input) {
  console.log("input", input);
  let { parcel_ids, response_type } = input;
  var myHeaders = {
  ...
  try {
    let response = await fetch(
      `${baseUrl}/${venderID}/api-get-standard-label-urls?vendorId=${venderID}`,
      requestOptions
    );
